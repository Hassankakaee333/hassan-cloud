"""Manual dispatcher for Frishta's post-security ACP sandbox benchmark.

This path is separate from Codex/provider execution. It accepts Binary distributions only and
launches a dedicated GitHub-hosted sandbox workflow after the API proves a prior artifact verification
job passed. The workflow receives no Hassan callback secret during Agent execution.
"""
...
WORKFLOW_FILE = "agent-acp-benchmark.yml"
...
class AgentBenchmarkSpec:
    ...
    def validate(self) -> None:
        ...
        if not self.security_verification_job_id or len(self.security_verification_job_id) > 128:
            raise ValueError("invalid security_verification_job_id")
...
def dispatch_agent_benchmark(
    ...
    *,
    ...
    repository = os.environ.get("HASSAN_AGENT_BENCHMARK_REPO", DEFAULT_REPO).strip() or DEFAULT_REPO
    ref = os.environ.get("HASSAN_AGENT_BENCHMARK_REF", "main").strip() or "main"
    if not token:
    ...
    if repository.count("/") != 1:
        raise ValueError("invalid HASSAN_AGENT_BENCHMARK_REPO")
    ...
    try:
        response = http.post(
            ...
            headers={
                ...
                "User-Agent": "Frishta-Agent-Benchmark-Dispatcher/1",
            },
            json=payload,
        )
