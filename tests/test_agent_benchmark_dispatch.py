...
from hassan_cloud.agent_benchmark import AgentBenchmarkSpec, dispatch_agent_benchmark  # noqa: E402


def spec(**overrides) -> AgentBenchmarkSpec:
    values = {
        "agent_id": "agent-x",
    ...
    return AgentBenchmarkSpec(**values)


def test_spec_rejects_command_escape_and_absolute_path() -> None:
    for command in ("../agent", "/usr/bin/agent", "C:\\agent.exe", "bin/../agent"):
        with pytest.raises(ValueError):
            spec(command=command).validate()


def test_spec_rejects_bad_sha_and_control_argument() -> None:
    with pytest.raises(ValueError):
...
def test_not_configured_is_honest(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = dispatch_agent_benchmark("job-1", spec())
    assert result.status == "NOT_CONFIGURED"
    assert "TOKEN" in result.detail


def test_dispatch_payload_is_structured_and_separate_from_codex(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("HASSAN_GITHUB_ACTIONS_TOKEN", "test-token")
    monkeypatch.setenv("HASSAN_AGENT_BENCHMARK_REPO", "owner/repo")
    monkeypatch.setenv("HASSAN_AGENT_BENCHMARK_REF", "candidate")
    captured = {}
    ...
    result = dispatch_agent_benchmark("benchmark-job-1", spec(), client=client)
    client.close()

    assert result.status == "QUEUED"
    assert result.workflow == "agent-acp-benchmark.yml"
    assert result.ref == "candidate"
    inputs = captured["body"]["inputs"]
    assert inputs["job_id"] == "benchmark-job-1"
